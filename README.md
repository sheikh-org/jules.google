...
Prompt
You ask Jules to take on an issue you don't want to do.

Plan
Jules analyzes the codebase and creates a plan.

Iterate
Jules writes code then tests it, screenshots, & more.

Code review
You review a Pull Request created by Jules.

Jules
Add onClick & disabled props
import React from 'react';
 
function MyButton({ text }) {
  const handleClick = () => {
    alert("Processing: " + text); // Initial basic action
  };
  return (
    <button
      className="btn-basic" // Initial class
      onClick={handleClick}>
      {text}
    </button>
  );
...
Writing softwareBike ridingPlaying tennis
...
Shows Work
Jules will show you its plan, provide reasoning, and give a diff of changes.
...
Priority access to the latest models, starting with Gemini 2.5 Pro
Try Jules
Documentation
Terms of Service
Privacy Policy
